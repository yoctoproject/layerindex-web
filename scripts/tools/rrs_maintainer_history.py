...
def get_recipe_maintainer(line, logger):
    ...
    if match:
        ...
        logger.debug("get_recipe_maintainer: %s, don\'t match" % (line))
        return None
...
def maintainer_history(logger):
    ...
    if not layer:
        self._logger.error("Core layer does not exist")
        sys.exit(1)
    ...
    pokypath = update_repo(settings.LAYER_FETCH_DIR, 'poky', settings.POKY_REPO_URL,
        True, logger)
    utils.runcmd("git checkout master", pokypath, logger=logger)
    ...
    for commit in commits.strip().split("\n"):
        ...
        utils.runcmd("git checkout %s -b %s" % (commit, branchname),
        ...
        for line in lines:
            ...
            if res:
                ...
                if qry:
                    ...
                    logger.debug("maintainer_history: not found recipe %s in %s" % \
                            (pn, layername))
